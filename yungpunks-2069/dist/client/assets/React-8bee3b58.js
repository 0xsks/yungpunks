import { c as map, t as tap, B as BehaviorSubject } from "./vendor-caa38ddf.js";
...
const RpgReactContext = reactExports.createContext({});
const useObjects = () => {
  const [objects, setObjects] = reactExports.useState([]);
  const {
    rpgObjects
  } = reactExports.useContext(RpgReactContext);
...
const useCurrentPlayer = () => {
  ...
  const currentPlayerRef = reactExports.useRef({});
  let _onChanges;
  ...
  return reactExports.useSyncExternalStore(subscribe, () => currentPlayerRef.current);
};
class ReactGui {
  ...
  constructor(rootEl, parentGui) {
    ...
    const GuiTooltip = (ui2) => {
      return () => {
        const [_tooltip, setTooltip] = reactExports.useState([]);
        reactExports.useEffect(() => {
          var _a;
        ...
        return parentGui.tooltipFilter(_tooltip).map((sprite) => reactExports.createElement("div", {
          style: parentGui.tooltipPosition({
            x: sprite.position.x,
            y: sprite.position.y
          }),
          key: sprite.id
        }, reactExports.createElement(ui2.gui, {
          spriteData: sprite,
          ...ui2.data || {}
        })));
      };
    ...
    const GuiWrapper = () => {
      const [_gui, setGui] = reactExports.useState([]);
      reactExports.useEffect(() => {
        this._gui.subscribe((gui) => setGui(gui));
      }, []);
      return reactExports.createElement(RpgReactContext.Provider, {
        value: parentGui.getInjectObject()
      }, ..._gui.filter((ui2) => ui2.display && !ui2.attachToSprite).map((ui2) => reactExports.createElement(ui2.gui, {
        key: ui2.name,
        ...ui2.data || {}
      })), ..._gui.filter((ui2) => ui2.display && ui2.attachToSprite).map((ui2) => reactExports.createElement("div", {
        key: ui2.name
      }, reactExports.createElement(GuiTooltip(ui2)))));
    };
    this.app.render(reactExports.createElement(GuiWrapper));
  }
  set gui(val) {
    let array = [];
    for (let key in val) {
